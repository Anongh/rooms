...
namespace eageramoeba.Lasers {
	...
	public class laserSystem : MonoBehaviour {
		...
		void FixedUpdate() {
			if (laserObj != null) {
				if (laserObj.GetComponent<laser>()) {
					if (onSys) {
						if (enablePool && simpleGOPool.instance == null) {
        ...
        void doHitM() {
            if (rendF = hit.collider.gameObject.GetComponent<MeshRenderer>()) {
                if (meshT = hit.collider.gameObject.GetComponent<MeshCollider>()) {
                    //get raycast material
                    tMat = new Material[1];
                    tMat[0] = getMaterial(hit, meshT, rendF);
                } else {
                    tMat = new Material[rendF.sharedMaterials.Length];
                    for (iM1 = 0; iM1 < rendF.sharedMaterials.Length; iM1++) {
                        tMat[iM1] = rendF.sharedMaterials[iM1];
                    }
                }
                //check if pass material applies
                for (iM = 0; iM < tMat.Length; iM++) {
                    if (l1.passReflMatMe) {
                        for (i2 = 0; i2 < l1.passedMatsMe.Length; i2++) {
                            if (tMat[iM] == l1.passedMatsMe[i2]) {
                                if (pasMatInc.Length >= i2) {
                                    l1.pasMatIncMe = pasMatInc[i2];
                                }
                                //if passthrough, do another scan after hit location
                                rayPass = true;
                            }
                        }
                    }
                    //check reflect material, if no passthrough
                    if (!rayPass && l1.hitReflMatMe) {
                        for (i2 = 0; i2 < l1.allowedMatsMe.Length; i2++) {
                            if (tMat[iM] == l1.allowedMatsMe[i2]) {
                                if (reflMatInc.Length >= i2) {
                                    l1.reflMatIncMe = reflMatInc[i2];
                                    if (l1.allowedMatsObjMe[i2] != null) {
                                        l1.hitGOM = l1.allowedMatsObjMe[i2];
                                    }
                                    l1.hitMat = tMat[iM];
                                }
                                reflMatT = true;
        ...
        void doHitM2D() {
            if (rendS = hit2D[0].collider.gameObject.GetComponent<SpriteRenderer>()) {
                tMat = new Material[rendS.sharedMaterials.Length];
                for (iM1 = 0; iM1 < rendS.sharedMaterials.Length; iM1++) {
                    tMat[iM1] = rendS.sharedMaterials[iM1];
                }
                goMat = true;
            } else if (rendF = hit2D[0].collider.gameObject.GetComponent<MeshRenderer>()) {
                tMat = new Material[rendF.sharedMaterials.Length];
                for (iM1 = 0; iM1 < rendF.sharedMaterials.Length; iM1++) {
                    tMat[iM1] = rendF.sharedMaterials[iM1];
                }
		...
		Material getMaterial(RaycastHit hit, MeshCollider collider, MeshRenderer renderer) {
		...
		GameObject createGO(GameObject go, Vector3 pos, Quaternion rot, lFxObj tFO) {
			GameObject tGo;
			if (enablePool) {
				tGo = simpleGOPool.instance.GetObjectForType(go.name);
